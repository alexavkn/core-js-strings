...
function removeFirstOccurrences(str, value) {
  let ind = 0;
  ind = str.indexOf(value);
  if (ind > -1) {
    return str.substr(0, ind).concat(str.substr(ind + value.length));
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  ...
  secondsStr = secondsStr.padStart(2, '0').padStart(3, ':');
...
function isPalindrome(str) {
  const strUpperCase = str.toUpperCase().replaceAll(/[\s \s!\s?\s,]/g, '');
  if (strUpperCase === strUpperCase.split('').reverse().join('')) {
    return true;
  }
  return false;
}
...
function invertCase(str) {
  ...
  arrWords = str.split(' ');
  for (let i = 0; i < arrWords.length; i += 1) {
    arrWord = arrWords[i].split('');
    for (let j = 0; j < arrWord.length; j += 1) {
      if (arrWord[j] === arrWord[j].toUpperCase()) {
        arrWord[j] = arrWord[j].toLowerCase();
      } else {
        arrWord[j] = arrWord[j].toUpperCase();
      }
    }
...
function extractNameFromTemplate(value) {
  ...
  arrValue = newValue.split(' ');
  return `${arrValue[1]} ${arrValue[2]}`;
}
...
function encodeToRot13(str) {
  ...
  let newStr = '';
  let numberInKey = -1;
  arrAlphabetU = 'ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ';
  arrAlphabetL = 'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
  arrAlphabetU = arrAlphabetU.split('');
  arrAlphabetL = arrAlphabetL.split('');
  for (let i = 0; i < arrStr.length; i += 1) {
    numberInKey = arrAlphabetU.indexOf(arrStr[i]);
    if (numberInKey === -1) {
      numberInKey = arrAlphabetL.indexOf(arrStr[i]);
      if (numberInKey === -1) {
        newStr += arrStr[i];
      } else {
        newStr += arrAlphabetL[numberInKey + 13];
      }
    } else {
      newStr += arrAlphabetU[numberInKey + 13];
    }
  }
  return newStr;
}
